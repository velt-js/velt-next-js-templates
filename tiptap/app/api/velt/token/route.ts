  ...
  try {
    ...
    if (!apiKey || !authToken) {
      return NextResponse.json(
        { error: 'Server configuration error: missing NEXT_PUBLIC_VELT_API_KEY or VELT_AUTH_TOKEN. See .env.example.' },
        { status: 500 }
      );
    }
    ...
    const res = await fetch('https://api.velt.dev/v2/auth/token/get', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-velt-api-key': apiKey, 'x-velt-auth-token': authToken },
